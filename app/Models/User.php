...
class User extends Authenticatable{
    ...
    public function ExistingRecordStaff($email,$mobile = NULL){
        $exists = NULL;
        if($mobile != NULL){
            $exists = $this::where('mobile',$mobile)->where('status','!=', 3)->exists();
        }
        if($exists == NULL){
            $exists = $this::where('email',$email)->where('status','!=', 3)->exists();
        }
		return $exists;
	}

	public function ExistingRecordUpdateStaff($email,$mobile = NULL, $id){
        $exists = NULL;
        if($mobile != NULL){
            $exists = $this::where('mobile',$mobile)->where('id','!=', $id)->where('status','!=', 3)->exists();
        }
        if($exists == NULL){
            $exists = $this::where('email',$email)->where('id','!=', $id)->where('status','!=', 3)->exists();
        }
		return $exists;
	}
